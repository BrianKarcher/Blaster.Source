...
namespace BlueOrb.Physics.SteeringBehaviors2D
{
    ...
    public static class SteeringBehaviorCalculations2
    {
        //------------------------------- Seek -----------------------------------
        //
        ...
        public static Vector2 Seek(Vector2 TargetPos, IPhysicsComponent entity)
        {
            ...
            var DesiredVelocity = (TargetPos - entity.GetFeetWorldPosition2());
            //if (DesiredVelocity.sqrMagnitude < 0.05f)
            //    return Vector2.zero;
            var desiredVelocityMaxSpeed = DesiredVelocity.normalized * entity.GetPhysicsData().MaxSpeed;
            var relativeVelocity = desiredVelocityMaxSpeed - entity.GetVelocity2();
            //if (relativeVelocity.sqrMagnitude < 0.05f)
            //    return Vector2.zero;

            return relativeVelocity;
        }

        public static Vector2 LateralWave(IPhysicsComponent entity, float time)
            => entity.transform.right.xz() * Mathf.Sin(Time.time * 3f);
        ...
        //    return desiredVelocityMaxSpeed;
        //}
        ...
        //------------------------------------------------------------------------
        public static Vector2 Flee(Vector2 TargetPos, IPhysicsComponent entity)
        {
            //var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            ...
            var DesiredVelocity = (entity.GetWorldPos2() - TargetPos).normalized
                                      * entity.GetPhysicsData().MaxSpeed;

            return (DesiredVelocity - entity.GetVelocity2());
        }
        ...
        //------------------------------------------------------------------------
        public static Vector2 Arrive(Vector2 TargetPos, IPhysicsComponent entity,
                                          Deceleration2 deceleration)
        {
            //var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            var ToTarget = TargetPos - entity.GetFeetWorldPosition2();
        ...
        public static Vector2 Pursuit(IPhysicsComponent evader, IPhysicsComponent entity)
        {
            ...
            var ToEvader = evader.GetWorldPos2() - entity.GetWorldPos2();
        ...
        //------------------------------------------------------------------------
        public static Vector2 Evade(IPhysicsComponent pursuer, IPhysicsComponent entity)
        {
            //var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            /* Not necessary to include the check for facing direction this time */

            Vector3 ToPursuer = pursuer.GetWorldPos2() - entity.GetWorldPos2();
            ...
            float LookAheadTime = ToPursuer.magnitude /
                                   (entity.GetPhysicsData().MaxSpeed + pursuer.GetVelocity2().magnitude);

            //now flee away from predicted future position of the pursuer
            return Flee(pursuer.GetFeetWorldPosition2() + pursuer.GetVelocity2() * LookAheadTime, entity);
        }
        ...
        //------------------------------------------------------------------------
        //public static Vector3 Wander(SpriteBase entity, ref Vector3 wanderTarget, float wanderJitter,
        ...
        //    wanderTarget += new Vector3(RandomClamped() * JitterThisTimeSlice,
        ...
        //    Vector3 target = wanderTarget + new Vector3(wanderDistance, 0);
        ...
        //    Vector3 Target = transform.TransformPoint(target.ToVector3(0f));
        ...
        //        Vector3 desired_velocity = to_wander_target * entity.GetPhysicsData().MaxSpeed;
        ...
        //        Vector3 steering_force = desired_velocity - entity.GetPhysicsData().Velocity;
        ...
        public static Vector2 RadiusClamp(IPhysicsComponent entity)
        {
            ...
            var projectedNearFutureLocation = entity.transform.position.xz() + entity.GetVelocity2();
            // TODO Fix this, this is a bug (distance is the same if inside or outside radius clamp)
            var distanceToFutureLocation = Vector3.Distance(projectedNearFutureLocation, target);
        ...
        public static Vector2 Wander(IPhysicsComponent entity, ref Vector2 wanderTarget, float wanderJitter,
            float wanderRadius, float wanderDistance, Transform transform, ref Vector2 last_wander_pos)
        {
            //this behavior is dependent on the update rate, so this line must
            //be included when using time independent framerate.
            float JitterThisTimeSlice = wanderJitter * Time.deltaTime;

            var deltaWanderTarget = new Vector2(RandomClamped() * JitterThisTimeSlice,
                                        RandomClamped() * JitterThisTimeSlice);
            ...
            //move the target into a position WanderDist in front of the agent
            Vector3 target = wanderTarget.xz() + new Vector3(0f, 0f, wanderDistance);
            ...
            //    entity.GetPhysicsData().Side = new Vector2(-heading.y, heading.x).xz();
            //}

            //project the target into world space
            //Vector2 Target = Transformations.PointToWorldSpace(target.xz(),
            //                                     entity.transform.forward.xz(),
            //                                     entity.transform.right.xz(),
            //                                     entity.GetWorldPos2());
            var Target = entity.transform.TransformPoint(target).xz();

            //calculate the center of the wander circle
            //Vector3 m_vTCC = PointToWorldSpace(Vector3(m_dWanderDistance * m_pVehicle->BRadius(), 0),
            //                                     m_pVehicle->Heading(),
            //                                     m_pVehicle->Side(),
            //                                     m_pVehicle->Pos());
            ...
            //gdi->RedPen();
            //gdi->Circle(PointToWorldSpace((m_vWanderTarget + Vector3(m_dWanderDistance, 0)) * m_pVehicle->BRadius(),
            //                              m_pVehicle->Heading(),
            //                              m_pVehicle->Side(),
            //                              m_pVehicle->Pos()), 3);
            ...
            //and steer towards it
            //var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            var force = Target - entity.GetFeetWorldPosition2();
        ...
        //    //move the target into a position WanderDist in front of the agent
        //    Vector3 target = m_vWanderTarget + Vector3(m_dWanderDistance, 0);
        ...
        //    circleCenter = new Vector3(heading.x, heading.y);
        ...
        public static Vector2 ObstacleAvoidance(IPhysicsComponent entity)
        //Vector3 SteeringBehavior::ObstacleAvoidance(const std::vector<BaseGameEntity*>& obstacles)
        {
            ...
            var m_dDBoxLength = MinDetectionBoxLength +
                            (entity.GetVelocity2().magnitude / entity.GetPhysicsData().MaxSpeed) *
            ...
            //{
            //IPhysicsComponent otherPhysicsComponent
            foreach (var ray in rays)
            {
                ...
                Vector2 LocalPos = Transformations.PointToLocalSpace(otherEntity.transform.position.xz(),
                                                       entity.transform.forward.xz(),
                                                       entity.transform.right.xz(),
                                                       entity.transform.position.xz());
            ...
            Vector2 finalForce = Transformations.VectorToWorldSpace(SteeringForce.xz(),
                                      entity.transform.forward.xz(),
                                      entity.transform.right.xz());
        ...
        //    {
        //      //calculate this obstacle's position in local space
        //      Vector3 LocalPos = PointToLocalSpace((*curOb)->Pos(),
        //                                             m_pVehicle->Heading(),
        //                                             m_pVehicle->Side(),
        //                                             m_pVehicle->Pos());
        ...
        //------------------------------------------------------------------------
        //public static Vector3 WallAvoidance()
        //{
        ...
        ////------------------------------------------------------------------------
        //public static IList<Vector3> CreateFeelers()
        //{
        ...
        //  //feeler to left
        //  Vector3 temp = m_pVehicle->Heading();
        ...
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::Separation(const vector<Vehicle*> &neighbors)
        //{  
        //  Vector3 SteeringForce;
        ...
        //    {
        //      Vector3 ToAgent = m_pVehicle->Pos() - neighbors[a]->Pos();
        ...
        //  of its neighbors
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::Alignment(const vector<Vehicle*>& neighbors)
        //{
        ...
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::Cohesion(const vector<Vehicle*> &neighbors)
        //{
        ...
        */
        ...
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::SeparationPlus(const vector<Vehicle*> &neighbors)
        //{  
        //  Vector3 SteeringForce;
        ...
        //    if(pV != m_pVehicle)
        //    {
        //      Vector3 ToAgent = m_pVehicle->Pos() - pV->Pos();
        ...
        //  USES SPACIAL PARTITIONING
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::AlignmentPlus(const vector<Vehicle*> &neighbors)
        //{
        ...
        //  USES SPACIAL PARTITIONING
        //------------------------------------------------------------------------
        //Vector3 SteeringBehavior::CohesionPlus(const vector<Vehicle*> &neighbors)
        //{
        //  //first find the center of mass of all the agents
        //  Vector3 CenterOfMass, SteeringForce;
        ...
        //------------------------------------------------------------------------
        public static Vector2 Interpose(IPhysicsComponent AgentA,
                                                ...
                                                IPhysicsComponent entity)
        {
            ...
            Vector2 MidPoint = (AgentA.GetWorldPos2() + AgentB.GetWorldPos2()) / 2.0f;

            float TimeToReachMidPoint = Vector2.Distance(entity.GetWorldPos2(), MidPoint) /
                                         entity.GetPhysicsData().MaxSpeed;

            //now we have T, we assume that agent A and agent B will continue on a
            //straight trajectory and extrapolate to get their future positions
            Vector2 APos = AgentA.GetWorldPos2() + AgentA.GetVelocity2() * TimeToReachMidPoint;
            Vector2 BPos = AgentB.GetWorldPos2() + AgentB.GetVelocity2() * TimeToReachMidPoint;

            //calculate the mid point of these predicted positions
            MidPoint = (APos + BPos) / 2.0f;

            //then steer to Arrive at it
            return Arrive(MidPoint, entity, Deceleration2.fast);
        }
        ...
        //------------------------------------------------------------------------
        public static Vector2 FollowPath(IPhysicsComponent entity, Path2 path)
        {
            //if (path.IsFinished())
        ...
        //------------------------------------------------------------------------
        public static Vector2 OffsetPursuit(IPhysicsComponent leader,
                                                  Vector2 offset, IPhysicsComponent entity)
        {
            //var physicsAffector = entity.GetSteering().SteeringPhysicsAffector;
            ...
            var WorldOffsetPos = leader.GetWorldPos2() + offset;
